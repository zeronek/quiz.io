...
var currPhoto = 0;                  // 현재 출력되는 사진의 시작 위치
var currPage = 1;                   // 현재 페이지
var totalPhoto = todayPhoto.length; //총 사진의 갯수
var numPhoto = 3;                   //화면에 출력되는 사진의 수
viewPhoto(currPhoto);
movePage(currPage);
...
prev.addEventListener('click', clickPrev);
    ...
    function clickNext(){
        if(currPhoto+numPhoto<todayPhoto.length){
            currPhoto = currPhoto + numPhoto;
            currPage = currPage + 1;
            viewPhoto(currPhoto);
            movePage(currPage);
    ...
    function viewPhoto(cp){
        for(var i=0; i<wrap.children.length; i++){
            wrap.removeChild[i];
        }

        var str = '';
        for(var i=cp; i<cp+numPhoto; i++){
            if(i>=0 && i<todayPhoto.length){
